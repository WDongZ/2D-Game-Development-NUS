...
public class SpeedDownStatus_B : Status
{
    ...
    public override void GetStatus(GameObject iObject)
    {
        if (iObject.tag == "Player" || iObject.tag == "Enemy")
        {
            ...
            speedDownEffect.GetComponent<SpeedDownStatusEffect_B>().lastingTime = lastingTime;
            iObject.GetComponent<EntityAttribute>().moveSpeed = iObject.GetComponent<EntityAttribute>().moveSpeed * 0.01f;
